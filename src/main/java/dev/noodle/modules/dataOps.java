...
public class dataOps {
    ...
    public static String getSqliteDatabaseURL() throws SQLException {
       return ("jdbc:sqlite:" +  getJarDir() + File.separator +"workingDb.db");
    }

    public static void createRecentQueryTable()  {
        Connection conn = null;
        try {
            conn = DriverManager.getConnection(getSqliteDatabaseURL());
...
//        }
    ...
    public static void importCSV(String FILENAME, String FILEPATH) throws IOException, InterruptedException, CsvValidationException, SQLException {
        ...
        String[] csvData = reader.readNext(); // Read the first row (data)
        Connection conn = DriverManager.getConnection(getSqliteDatabaseURL());
        Statement stmt = conn.createStatement();

        stmt.executeUpdate("DROP TABLE IF EXISTS [" + FILENAME + "]");
            ...
            insertSQL.append(") VALUES (");
            insertSQL.append("?,".repeat(csvData.length).substring(0, csvData.length * 2 - 1)).append(")");
            //System.out.println(insertSQL);

            try (PreparedStatement pstmt = conn.prepareStatement(insertSQL.toString())) {
                String[] row;
                int batchSize = 10000; // Adjust batch size for large CSVs
                int count = 0;
    ...
    public static TableModel<String> tableModelFromInternalSQLTable(String FILENAME) throws SQLException {
        ...
        try (Connection conn = DriverManager.getConnection(getSqliteDatabaseURL());
             Statement stmt = conn.createStatement();
            ...
            for (int i = 1; i <= columnCount; i++) { // must start at 1 for lanterna table indexing
    ...
    public static RadioBoxList<remoteSQL.OptionofRecent> recentTableEntriesListFromInternal(String selectCol, String tableName, int recentResultsToShow) {
        ...
        if (!Objects.equals(selectCol, "*")) {
            // absolutely will not work if you input * outside the method as selectCol without this if statement????????
            try (Connection conn = DriverManager.getConnection(getSqliteDatabaseURL());
                 Statement stmt = conn.createStatement();
